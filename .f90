...
public :: compute_ctprime, compute_induction
public :: compute_ct_local, CT_of_Uinf, U_inf_from_Ud, Udtr_from_Utr
public :: ctprime_init
...
real(rprec), public :: alpha_fit = 0.7_rprec         ! This fit parameter is an Input
real(rprec), private :: beta_fit  = 1.0_rprec        ! This fit parameter is not user-defined

!-----------------------------------------------------------
! TSR fitting parameters (computed in ctprime_init)
real(rprec), private :: P, Q

real(rprec), private :: eps_safe
...
! Compute induction factor a based on disk-averaged velocity and pitch
  ...
  Ud_tr = 0.5_rprec * U_trans * (1.0_rprec + sqrt(max(1.0_rprec - CT_op*cos(theta), eps_safe)))

  if (Ud <= Ud_tr) then
     ! Region II
     a = (1.0_rprec - sqrt(1.0_rprec - CT_op*cos(theta))) / 2.0_rprec
  else
     ! Region III
     num = 4.0_rprec*alpha*Ud - CT_op*cos(theta)
     term1 = alpha*U_trans + alpha*Ud - 1.0_rprec
     term2 = term1**2 - (4.0_rprec*alpha*Ud - CT_op*cos(theta))*(alpha*U_trans - 1.0_rprec)
...
! Compute transitional disk-averaged velocity
  ...
  Udtr = 0.5_rprec * Utr * (1.0_rprec + sqrt(max(1.0_rprec - CT_op*cos(theta), eps_safe)))
...
! Compute local thrust coefficient C'_T from scalar CT and azimuth angle theta
  ...
  sqrt_term = sqrt(max(1.0_rprec - CT*cos(theta), eps_safe))
